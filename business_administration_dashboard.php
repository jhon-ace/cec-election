    ...
    for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
      {
          ...
          $gov_vice_ba = $_POST['ba_vice_gov'];
           ...
           $sql2 = "INSERT into governors(gov_vice_id,stud_eyeD,department,governor,vice_governor,year) VALUES('','$student_id','BSBA','$gov_ba','$gov_vice_ba','ALL BSBA')";
              mysqli_query($db,$sql2);

                    $message = "Congratulation! Successful Voting";
                        echo "<script type='text/javascript'>alert('$message'); window.location.assign('business_administration_dashboard.php')</script>";
      }
  }
              ...
              if (mysqli_num_rows($res_u) > 0) 
              {
                $message = "Senator Already Selected";
                  echo "<script type='text/javascript'>alert('$message');
                    window.location.assign('business_administration_dashboard.php')</script>"; 
              }
              else if( $alllogs < $max) 
              {
...
<body style="background-color:white">
  ...
  ?>
    <form method="POST"> 
        ...
        <div class='media-body text-left'>
          <input type = "hidden" name ="stud_idd" value='<?=$student;?>'>
          <h4>Hospitality and Tourism Management Program | Governor</h4>
            <?php
                  $Student = $_SESSION['login_name'];
      ...
      <div class='media col-md-12'>
        <div class='media-body text-left'>
          <h4>Hospitality and Tourism Management Program | Vice - Governor</h4>
            <?php
                  $Student = $_SESSION['login_name'];
                  $sql = mysqli_query($link,"SELECT  * FROM governors where stud_eyeD = '$student'");
                    echo "<select name='grade12_governor' class='form-control' disabled>";
                            ...
                            <h4>For Business Administration Program | Governor</h4>
                                <?php
                                    $sql = mysqli_query($link,"SELECT  * FROM candidates where can_position = 'Governor' AND can_department = 'Business Administration'");
                                        ...
                                        echo "<select name='ba_gov' class='form-control'  onchange='getData(this);' id='form_frame7'>";
                                      echo "<option>No Business Administration Governor selected</option>";
                                        for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
                                            {
                                              $lastname = $num_rows['can_lastname'];
                                              $firstname = $num_rows['can_firstname'];
                            ...
                            <h4>For Business Administration Program | Vice - Governor</h4>
                                <?php
                                    $sql = mysqli_query($link,"SELECT  * FROM candidates where can_position = 'Vice - Governor' AND can_department = 'Business Administration'");
                                        ...
                                        echo "<select name='ba_vice_gov' class='form-control'  onchange='getData(this);' id='form_frame8'>";
                                      echo "<option>No Business Administration Vice - Governor selected</option>";
                                        for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
                                            {
                                              $lastname = $num_rows['can_lastname'];
                                              $firstname = $num_rows['can_firstname'];

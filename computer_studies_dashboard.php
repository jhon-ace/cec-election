    ...
    for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
      {
          ...
          $gov_bsit = $_POST['bsit_gov'];
          $gov_vice_bsit = $_POST['bsit_vice_gov'];
           ...
           $sql2 = "INSERT into governors(gov_vice_id,stud_eyeD,department,governor,vice_governor,year) VALUES('','$student_id','BSIT','$gov_bsit','$gov_vice_bsit','ALL BSIT')";
              mysqli_query($db,$sql2);

                    $message = "Congratulation! Successful Voting";
                        echo "<script type='text/javascript'>alert('$message'); window.location.assign('computer_studies_dashboard.php')</script>";
      }
  }
              ...
              if (mysqli_num_rows($res_u) > 0) 
              {
                $message = "Senator Already Selected";
                  echo "<script type='text/javascript'>alert('$message');
                    window.location.assign('computer_studies_dashboard.php')</script>"; 
              }
              else if( $alllogs < $max) 
              {
...
<html lang="en">
  <head>
    ...
    <title>Computer Studies Dept | Voting Page</title>
...
<body style="background-color:white">
  ...
  ?>
    <form method="POST"> 
              ...
              <table id="tableCart" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%" style="background-color: white;">  
                <thead>   
                  <tr> 
                    ...
                    <th>List of Senator selected <i class="fa fa-arrow-down" style="color:red"></i> <i class="fa fa-check" style="color: green"></i></th>
                  </tr>  
                </thead>
                <tbody>
                  <input type = "hidden" name ="stud_idd" value='<?=$student;?>'>
        ...
        <div class='media-body text-left'>
          <input type = "hidden" name ="stud_idd" value='<?=$student;?>'>
          <h4>Computer Studies Department | Governor</h4>
            <?php
                  $Student = $_SESSION['login_name'];
      ...
      <div class='media col-md-12'>
        <div class='media-body text-left'>
          <h4>Computer Studies Department | Vice - Governor</h4>
            <?php
                  $Student = $_SESSION['login_name'];
                  $sql = mysqli_query($link,"SELECT  * FROM governors where stud_eyeD = '$student'");
                    echo "<select name='grade12_governor' class='form-control' disabled>";
              ...
              <div class="container" style="padding: 5px 0; float: left; width: 100%;">
                <table id="tableCart" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%" style="background-color: white;">  
                  <thead>   
                    <tr> 
                      <th style="display: none">ID</th>
                      <th>List of selected senator <i class="fa fa-arrow-down" style="color:red"></i></th>
                      <th>Option</th>
                    </tr>  
                  </thead>
                    <tbody>
        ...
        <div class='media-body text-left'>
          <h4>For Computer Studies | Governor</h4>
            <?php
              ...
              echo "<select name='bsit_gov' class='form-control' onchange='getData(this);' id='form_frame7'>";
              echo "<option>No Computer Studies Governor selected</option>";
                  for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
                  {
                      $lastname = $num_rows['can_lastname'];
                      $firstname = $num_rows['can_firstname'];
          ...
          <h4>For Computer Studies | Vice - Governor</h4>
            <?php
             $sql = mysqli_query($link,"SELECT  * FROM candidates where can_position = 'Vice - Governor' AND can_department = 'Computer Studies'");                 
                echo "<select name='bsit_vice_gov' class='form-control' onchange='getData(this);' id='form_frame8'>";
                echo "<option>No Computer Studies Vice - Governor selected</option>";
                  for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
                  {
                      $lastname = $num_rows['can_lastname'];
                      $firstname = $num_rows['can_firstname'];

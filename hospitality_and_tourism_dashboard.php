    ...
    for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
      {
          ...
          $gov_hmtm = $_POST['hm_tm_gov'];
          $gov_vice_hmtm = $_POST['hm_tm_vice_gov'];
           ...
           $sql2 = "INSERT into governors(gov_vice_id,stud_eyeD,department,governor,vice_governor,year) VALUES('','$student_id','BSHM | BSTM','$gov_hmtm','$gov_vice_hmtm','ALL BSHM & BSTM')";
              mysqli_query($db,$sql2);

                    $message = "Congratulation! Successful Voting";
                        echo "<script type='text/javascript'>alert('$message'); window.location.assign('hospitality_and_tourism_dashboard.php')</script>";
      }
  }
              ...
              if (mysqli_num_rows($res_u) > 0) 
              {
                $message = "Senator Already Selected";
                  echo "<script type='text/javascript'>alert('$message');
                    window.location.assign('hospitality_and_tourism_dashboard.php')</script>"; 
              }
              else if( $alllogs < $max) 
              {
...
<html lang="en">
  <head>
    ...
    <title>BSHM | BSTM | Voting Page</title>
                                    ...
                                    <h4>For Hospitality and Tourism Management Program | Governor</h4>
                                        <?php
                                            $sql = mysqli_query($link,"SELECT  * FROM candidates where can_position = 'Governor' AND can_department = 'Hospitality and Tourism'");
                                                ...
                                                echo "<select name='hm_tm_gov' class='form-control' onchange='getData(this);' id='form_frame7'>";
                                              echo "<option>No BSHM & BSTM Governor selected</option>";
                                                for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
                                                    {
                                                      $lastname = $num_rows['can_lastname'];
                                                      $firstname = $num_rows['can_firstname'];
                            ...
                            <h4>For Hospitality and Tourism Management Program | Vice - Governor</h4>
                                <?php
                                    $sql = mysqli_query($link,"SELECT  * FROM candidates where can_position = 'Vice - Governor' AND can_department = 'Hospitality and Tourism'");
                                        ...
                                        echo "<select name='hm_tm_vice_gov' class='form-control' onchange='getData(this);' id='form_frame8'>";
                                      echo "<option>No BSHM & BSTM Vice - Governor selected</option>";
                                        for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
                                            {
                                              $lastname = $num_rows['can_lastname'];
                                              $firstname = $num_rows['can_firstname'];

    ...
    for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
      {
          ...
          $update_grand_total = "UPDATE students SET stud_status = '1', voting_status = 'finish voting', stud_block_status = 'none' WHERE stud_id ='$stud_id'";
           ...
           $sql2 = "INSERT into governors(gov_vice_id,stud_eyeD,department,governor,vice_governor,year) VALUES('','$student_id','G - 12','$governor12','$vice_governor12','G - 12')";
              mysqli_query($db,$sql2);
                    ...
                    $message = "Congratulation! Successful Voting";
                        echo "<script type='text/javascript'>alert('$message'); window.location.assign('grade12_dashboard.php')</script>";
      }
  }
              ...
              if (mysqli_num_rows($res_u) > 0) 
              {
                $message = "Senator Already Selected";
                  echo "<script type='text/javascript'>alert('$message');
                    window.location.assign('grade12_dashboard.php')</script>"; 
              }
              else if( $alllogs < $max) 
              {
...
<html lang="en">
  <head>
    ...
    <title>GRADE - 12 | Voting Page</title>
    ...
    <nav class="navbar navbar-inverse" style="border-radius: 0">
      <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="myNavbar" style="float: right;margin-top: 0px">
          ...
          <form method="POST">
            <li><button class="btn btn-danger"  type='button' data-toggle="modal" data-target="#exampleModal20000" style="margin-top: 8px;margin-right: 20px"><i class="fa fa-power-off" style="color:white"></i> Signout</button></li>
          </form>
          </ul>
        </div>
      </div>
    ...
    ?>
          ...
          ?>
              <section class="jumbotron my-1 text-left" style="margin-top: -50px;">
                  <div class="container">
                    <h3 class="jumbotron-heading text-center" style="font-family: sans-serif;">CSG Election | 2020</h3><br>
                   <p class="text-center" style="font-size: 18px;color:red;border:0;margin-top: -15px">Here are the official list of your votes.
                   </p><br>
                   <p class="text-center" style="font-size: 15px;color:red;margin-top: -30px">*Please approach the election officer that you are finish voting for clearance signing.*<br> Thank You.</p>
                  </div>
              </section>
     <?php } ?>
    <?php } ?>
              ...
              <table id="tableCart" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%" style="background-color: white;">  
                <thead>   
                  <tr> 
                    ...
                    <th>Senator <i class="fa fa-check" style="color: green"></i><text style="float:right"> You select <text id="countsenatorselected" style="color:green"></text> out of 8 senators to be selected.</text></th>
                  </tr>  
                </thead>
                <tbody>
                  <input type = "hidden" name ="stud_idd" value='<?=$student;?>'>
            ...
            <h4>For Senator | Max : 8</h4>
              <?php
                $sql = mysqli_query($link,"SELECT  * FROM candidates where can_position = 'Senator'");
                  ...
                  echo "</select>";
                    echo "<button class='btn btn-primary btn-lg ' name='senator-add' style='float:right;margin-top:4px '>Select</button>";
                      echo "<br>";
              ?>
              ...
              <div class="container" style="padding: 5px 0; float: left; width: 100%;">
                <table id="tableCart" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%" style="background-color: white;">  
                  <thead>   
                    <tr> 
                      <th style="display: none">ID</th>
                      <th>Senator  <text style="float: right"> You select <text id="countsenatorselected" style="color:red"></text> out of 8 senators to be selected.</text></th>
                      <th>Option</th>
                    </tr>  
                  </thead>
                    <tbody>
          ...
          <div class='media-body text-left'>
            <input type = 'hidden' name ='stud_idd' value='<?=$student;?>'>
              <h4>For Grade - 12 | Vice - Governor</h4>
                ...
                ?>
                <br><br>
                  <input type="submit" class="btn btn-danger btn-lg btn-block mb-4 mt-4 acebtn" onclick="return confirm('Are you sure you want to submit your votes?')" value="SUBMIT VOTE" name="submit-vote"/>
            </div>   
          </div>
    </form>
...
$(function() {
    if (localStorage.getItem('form_frame4')) {
        $("#form_frame4 option").eq(localStorage.getItem('form_frame3')).prop('selected', true);
    }
...
<script>
      $(document).ready(function(){
          setInterval(function(){
            $("#countsenatorselected").load('senator_count.php')
          }, 200);
        });
    </script>

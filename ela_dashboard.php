    ...
    for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
      {
          ...
          $govela = $_POST['ela_gov'];
          $govviceela = $_POST['ela_vice_gov'];

           $sql = "INSERT into votes(id,stud_id,president,vice_president,secretary,treasurer,auditor,student_voting_status) VALUES('','$student_id','$pres','$vice_pres','$secretary','$treasurer','$auditor','finished voting')";
              mysqli_query($db,$sql);

           $sql2 = "INSERT into governors(gov_vice_id,stud_eyeD,department,governor,vice_governor,year) VALUES('','$student_id','Education and Liberal Arts','$govela','$govviceela','ALL Educ and Liberal Arts')";
              mysqli_query($db,$sql2);

                    $message = "Congratulation! Successful Voting";
                        echo "<script type='text/javascript'>alert('$message'); window.location.assign('ela_dashboard.php')</script>";
      }
  }
              ...
              if (mysqli_num_rows($res_u) > 0) 
              {
                $message = "Senator Already Selected";
                  echo "<script type='text/javascript'>alert('$message');
                    window.location.assign('ela_dashboard.php')</script>"; 
              }
              else if( $alllogs < $max) 
              {
...
<html lang="en">
  <head>
    ...
    <title>E.L.A Department | Voting Page</title>
...
<body style="background-color:white">
  ...
  ?>
    <form method="POST"> 
              ...
              <table id="tableCart" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%" style="background-color: white;">  
                <thead>   
                  <tr> 
                    ...
                    <th>Lists of Senator selected <i class="fa fa-arrow-down" style="color:red"></i> <i class="fa fa-check" style="color: green"></i></th>
                  </tr>  
                </thead>
                <tbody>
                  <input type = "hidden" name ="stud_idd" value='<?=$student;?>'>
        ...
        <div class='media-body text-left'>
          <input type = "hidden" name ="stud_idd" value='<?=$student;?>'>
          <h4>Education and Liberal Arts Department | Governor</h4>
            <?php
                  $Student = $_SESSION['login_name'];
      ...
      <div class='media col-md-12'>
        <div class='media-body text-left'>
          <h4>Education and Liberal Arts Department | Vice - Governor</h4>
            <?php
                  $Student = $_SESSION['login_name'];
                  $sql = mysqli_query($link,"SELECT  * FROM governors where stud_eyeD = '$student'");
                    echo "<select name='grade12_governor' class='form-control' disabled>";
              ...
              <div class="container" style="padding: 5px 0; float: left; width: 100%;">
                <table id="tableCart" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%" style="background-color: white;">  
                  <thead>   
                    <tr> 
                      <th style="display: none">ID</th>
                      <th>Lists of Senator selected <i class="fa fa-arrow-down" style="color:red"></i></th>
                      <th>Option</th>
                    </tr>  
                  </thead>
                    <tbody>
                            ...
                            <h4>For Education and Liberal Arts | Governor</h4>
                                <?php
                                    $sql = mysqli_query($link,"SELECT  * FROM candidates where can_position = 'Governor' AND can_department = 'Education and Liberal Arts'");
                                        ...
                                        echo "<select name='ela_gov' class='form-control' onchange='getData(this);' id='form_frame7'>";
                                      echo "<option>No Education and Liberal Arts Governor selected</option>";
                                        for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
                                            {
                                              $lastname = $num_rows['can_lastname'];
                                              $firstname = $num_rows['can_firstname'];
                            ...
                            <h4>For Education and Liberal Arts | Vice - Governor</h4>
                                <?php
                                    $sql = mysqli_query($link,"SELECT  * FROM candidates where can_position = 'Vice - Governor' AND can_department = 'Education and Liberal Arts'");
                                        ...
                                        echo "<select name='ela_vice_gov' class='form-control' onchange='getData(this);' id='form_frame8'>";
                                      echo "<option>No Education & Liberal Arts Vice - Governor selected</option>";
                                        for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
                                            {
                                              $lastname = $num_rows['can_lastname'];
                                              $firstname = $num_rows['can_firstname'];

...
class myPDF extends FPDF{
		...
		$this->SetFont('Arial','B',10);
            ...
            $vot = "finish voting";
                $sql2 = mysqli_query($link, "SELECT COUNT(*) 'TOTAL' FROM students WHERE voting_status = 'ready to vote' AND stud_year = 'G - 12' AND stud_status = '0' and stud_block_status = 'unblock'");
                     $res2 = mysqli_fetch_array($sql2);
                        $alllogs = $res2['TOTAL'];


		$this->Cell(77,5,'GRADE - 12 | LIST OF REMAINING VOTERS',0,0,'R');
	...
	function viewTable($db){
		...
		$link = mysqli_connect('localhost','root','19976112','election');
		$sql = mysqli_query($link,"SELECT * FROM students where voting_status = 'ready to vote' AND stud_block_status = 'unblock' AND stud_year = 'G - 12' and stud_status = '0' ORDER BY stud_department,stud_lastname ASC");
...
date_default_timezone_set('Asia/Manila');
//$datekaron = date('l, F j, Y');
$pdf->Output('I','Grade - 12 | Remaining Voters.pdf','I');

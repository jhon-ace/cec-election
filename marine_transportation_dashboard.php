    ...
    for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
      {
          ...
          $gov_vice_MT = $_POST['mt_vice_gov'];
           ...
           $sql2 = "INSERT into governors(gov_vice_id,stud_eyeD,department,governor,vice_governor,year) VALUES('','$student_id','BSMT','$gov_MT','$gov_vice_MT','ALL BSMT')";
              mysqli_query($db,$sql2);

                    $message = "Congratulation! Successful Voting";
                        echo "<script type='text/javascript'>alert('$message'); window.location.assign('marine_transportation_dashboard.php')</script>";
      }
  }
              ...
              if (mysqli_num_rows($res_u) > 0) 
              {
                $message = "Senator Already Selected";
                  echo "<script type='text/javascript'>alert('$message');
                    window.location.assign('marine_transportation_dashboard.php')</script>"; 
              }
              else if( $alllogs < $max) 
              {
...
<html lang="en">
  <head>
    ...
    <title>Marine Transportation Program | Voting Page</title>
...
<body style="background-color:white">
  ...
  ?>
    <form method="POST"> 
        ...
        <div class='media-body text-left'>
          <input type = "hidden" name ="stud_idd" value='<?=$student;?>'>
          <h4>Marine Transportation Program | Governor</h4>
            <?php
                  $Student = $_SESSION['login_name'];
      ...
      <div class='media col-md-12'>
        <div class='media-body text-left'>
          <h4>Marine Transportation Program | Vice - Governor</h4>
            <?php
                  $Student = $_SESSION['login_name'];
                  $sql = mysqli_query($link,"SELECT  * FROM governors where stud_eyeD = '$student'");
                    echo "<select name='grade12_governor' class='form-control' disabled>";
        ...
        <div class='media-body text-left'>
          <h4>For Marine Transportation Program | Governor</h4>
            <?php
              ...
              echo "<select name='mt_gov' class='form-control' onchange='getData(this);' id='form_frame7'>";
                echo "<option>No Marine Transportation Governor selected</option>";
                  for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
                    {
                      $lastname = $num_rows['can_lastname'];
                      $firstname = $num_rows['can_firstname'];
        ...
        <div class='media-body text-left'>
          <h4>For Marine Transportation Program | Vice - Governor</h4>
            <?php
              ...
              echo "<select name='mt_vice_gov' class='form-control' onchange='getData(this);' id='form_frame8'>";
                echo "<option>No Marine Transportation Vice - Governor selected</option>";
                  for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
                  {
                    $lastname = $num_rows['can_lastname'];
                    $firstname = $num_rows['can_firstname'];
